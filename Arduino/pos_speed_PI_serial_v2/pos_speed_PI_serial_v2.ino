...
float xo = 5.0;
float yo = 0.0;
// Defina os pesos para cada fonte de dados
const float odometry_weight = 0.3;  // Peso para o valor de theta calculado por odometria
const float gyro_weight = 0.7;      // Peso para o valor de theta medido pelo giroscópio

// Variáveis para armazenar os valores de theta
float theta_odometry = 0;
float theta_gyro = 0;
...
//static control_t control_left_omega = init_control( 15, 0.15, 0.05) ;   	// direction internal speed control
//static control_t control_left_theta = init_control_traj( 2, 0) ;      	// direction external position control 

//static control_t control_right_omega = init_control( 10, 20, 0.05 ) ;   // power internal speed control
...
const int state_size = 2;   // Tamanho do vetor de estado [theta, velocidade_theta]
const int measurement_size = 1;  // Tamanho do vetor de medição [theta]
...
float fusion_theta(float odometry_theta, float gyro_theta) {
    // Calcule a média ponderada dos valores de theta
    return (odometry_weight * odometry_theta) + (gyro_weight * gyro_theta);
}
...
void loop()
{
  ...
  if (c == 'U' || c == 'w') {
	...
	*/
    ...
    // Restante do código // Tamanho do buffer deve ser grande o suficiente para armazenar ambos os valores formatados

// Formata os valores para uma string
	// sends bin data
	//Serial2.write( (byte *) &mobile_robot, sizeof(robot_t) );	
...
ISR(TIMER5_COMPA_vect){ 
  ...
  if( Serial2.available() > 0 ){
    for(int i = 0 ; i < 50 ; i++){
      ...
      if( input == '\n' || input == '\0' || Serial2.available() == 0 ){
        break;
      } 
    }
    Serial.println(buffer_serial);
    
    val1 = atof(buffer_serial);
    val2 = atof(&buffer_serial[4]);
    val3 = atof(&buffer_serial[8]);
  ...
  encoder_left.odom = -1*encoder_lib_left.read() ;
	...
	time_counter += 0.010 ; 		// time_counter is in seconds
	    ...
	    params.L = 0.3; // Comprimento do eixo
	    params.x_pos = x; // Posição x atual
	    params.y_pos = y; // Posição y atual
	    params.theta_pos = theta_gyro; // Orientação atual

      params.xd = val1/5; // Destino x
      params.yd = val2/5 ; // Destino y
      params.thetad = val3; // Orientação desejada

      /*
	    params.xd = values[0]; // Destino x
	    params.yd = values[1]; // Destino y
	    params.thetad = values[2]; // Orientação desejada
      */
      
    // Controlador da trajetória
    WheelVelocities wheel_velocities = controlTrajectory(params);
    ...
    drive_voltage(motor_right,wheel_velocities.ur);
    drive_voltage(motor_left,-wheel_velocities.ul);
    ...
    dot_x = v * cos(theta_gyro);
    dot_y = v * sin(theta_gyro);
    dot_theta = velocity_angular();

    x += dot_x*delta_t;
    y += dot_y*delta_t;
    theta_odometry += dot_theta*delta_t;

    gyro.update();
    theta_gyro = gyro.getAngleZ()*PI/180;

    fused_theta = fusion_theta(theta_odometry,theta_gyro);  // Usa tanto a odometria quanto as leituras do giroscópio para a atualização do filtro

    //if ((params.xd - x) < 0.02 && (params.yd - y) < 0.02 ) {
    //  Serial.write(1);
    ...
    Serial.print(y);
    Serial.print(" Theta :");
    Serial.print(fused_theta);
    Serial.print(" Vlinear :");
    Serial.print(velocity_linear());
    Serial.print(" Vangular :");
    Serial.println(velocity_angular());
    Serial.print(" Command left:");
    Serial.print(wheel_velocities.ul);
    Serial.print(" Commande right :");
    Serial.println(wheel_velocities.ur);
    ...
    char c = Serial.read();
    if (c == 'U' || c == 'w') {
      drive_voltage(motor_right,3.0);
      drive_voltage(motor_left,-3.0);
    } else if (c == 'D' || c == 's') {
