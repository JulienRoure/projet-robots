...
void calculate_speed( encoder_t * encoder ){
    ...
    if( encoder == &encoder_right){
        encoder->omega = ( (float)delta * 0.0008536684 );    //  ( ( 2 * PI ) / ( 46 * 1 * 8 * 2 ) ) );   
    }
    else if( encoder == &encoder_left){
      encoder->omega = ( (float)delta *  0.0008536684  );    //  ( ( 2 * PI ) / ( 75 * 1 * 8 * 4 ) ) );    0.0609781 
    }
...
void calculate_pos( encoder_t * encoder){
    ...
    if( encoder == &encoder_right){
      //encoder->theta = ( (float)encoder->odom * ( ( 2 * PI ) / ( 46 * 7 * 8 * 4 ) ) );     
        encoder->theta = ( (float)encoder->odom * 0.008536684 );     
    }
    else if( encoder == &encoder_left){
      //encoder->theta = ( (float)encoder->odom * ( ( 2 * PI ) / ( 46 * 7 * 8 * 4 ) ) );          
      encoder->theta = ( (float)encoder->odom * 0.008536684); 
...
float velocity_linear() {
    ...
    float v = (encoder_right.omega + encoder_left.omega)*0.029/2;
    return v;
  }

float velocity_angular() {
    float baseRobot = 0.18;
    float w = (encoder_right.omega - encoder_left.omega)*0.029/baseRobot;
    return w;
  }
