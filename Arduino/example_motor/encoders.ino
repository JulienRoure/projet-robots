...
void power_isr() {
    ...
    if( digitalRead( encoder_right.B ) ){     // checks for polarity
        encoder_right.odom += 1.0000 ;
    }
    else{
        encoder_right.odom -= 1.0500 ;  
    }
...
void direc_isr() {
    ...
    if( digitalRead( encoder_left.B ) ){     // checks for polarity
        encoder_left.odom += 1.0000 * 0.82 ;
    }
    else{
        encoder_left.odom -= (1.0000 + 0.00620) * 0.82 ;  
    }
