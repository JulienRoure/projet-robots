...
void write_in_file(char *message, char *nom_fichier) {#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <arpa/inet.h>
...
int main() {
	...
	char* fichierCommande[NB_MAX_CONNECTIONS];
    ...
    for(int i = 0; i < NB_MAX_CONNECTIONS; i++) {
        ...
        if (pidFils[i] == 0) {
			...
			while(1){
		        ...
		        for (int j = 0; j < NB_MAX_CONNECTIONS; j++) {
		            //sprintf(bufferEmission, "Message n°%d adressé au client n°%d\n", j, i);
		            //send(socketDialogue[i], bufferEmission, strlen(bufferEmission), 0);
		            bufferEmission = lireFichier(fichierCommande[j]);
		        	send(socketDialogue[i], bufferEmission, strlen(bufferEmission), 0);
